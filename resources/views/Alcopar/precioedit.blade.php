...
<section id="basic-datatable">
	<div class="row">
		<div class="col-sm-12">
			<h2><strong>Revisión de Número de Parte - Alta Precio</strong></h2>
		</div>
	</div>
</section>
...
function historialcargar(id){
        ...
        $("#panel").load("{{ url('/alcopar/historial/')}}/"+id,  function() {
            $('.loaderhist').hide();
            $('#panel').show();
        });
	}
        ...
        $("#categoria").change(function () {
            $("#categoria option:selected").each(function () {
                let id_categoria = $(this).val();
                    $("#familia").load("{{ url('/alcopar/reving/jquery/getFamiliaJquery/')}}/"+id_categoria,  function() {
                    });
            });
        });
